...
const About = () => {
  return (
    <div className="w-full font-sans">
      {/* Hero Section */}
      <section
        className="relative h-[60vh] md:h-[80vh] bg-cover bg-center bg-fixed flex items-center justify-center text-white text-center px-4"
        style={{
          backgroundImage:
      ...
      <section className="py-16 px-6 bg-white text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {[
            { title: 'Innovation', desc: 'We constantly explore new ideas to deliver the best digital solutions.' },
            { title: 'Quality', desc: 'We focus on clean code, modern design, and high-performance apps.' },
            { title: 'Collaboration', desc: 'We work closely with our clients to understand and deliver their vision.' }
          ].map((item, i) => (
            <div key={i} className="bg-gray-50 p-6 rounded-xl shadow hover:-translate-y-1 transition">
...
<section className="py-16 px-6 text-center bg-gray-50">
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 max-w-4xl mx-auto">
    {[
      ...
      <div
        key={index}
        className="bg-white w-full max-w-xs rounded-2xl shadow-lg mx-auto p-6 text-left hover:-translate-y-1 transition"
      >
        <img
          src={member.image}
          alt={member.name}
